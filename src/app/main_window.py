...
def _remove_dwm_shadow(window):
    """
    Windows 平台: 移除 DWM 为 frameless 窗口自动添加的阴影边框。
    在 Windows 上，即使设置了 FramelessWindowHint + WA_TranslucentBackground，
    DWM 仍会在窗口边缘绘制一层半透明的阴影边框（Windows 10/11）。
    通过 DwmExtendFrameIntoClientArea 将 frame 扩展到整个客户区，
    使 DWM 不再单独绘制窗口边框。
    """
    ...
    try:
        ...
        logger.info("已移除 Windows DWM 窗口阴影边框")
    except Exception as e:
        logger.warning(f"移除 Windows DWM 窗口阴影失败: {e}")
